...
import { ApplicationCommandType, ButtonBuilder, ActionRowBuilder, ButtonStyle, MessageFlags, } from "discord.js";
import { QuickDB } from "quick.db";
const db = new QuickDB();
const config = db.table("configAuxiliar");
// helpers para não repetir código
const buildRow = () => new ActionRowBuilder().addComponents(new ButtonBuilder()
    .setLabel("Adicionar")
...
createResponder({
    ...
    async run(interaction) {
        ...
        if (atual > 0)
            await config.sub("valorTotalTicket", 1); // evita negativo
